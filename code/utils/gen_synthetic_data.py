...
def homographyGeneration(args, raw_image_path, index):
  ...
  if args.mode=='train' and not args.debug:
    f_pts1      = open(args.pts1_file, 'a')
    f_gt        = open(args.gt_file,   'a')
    f_file_list = open(args.filenames_file, 'a')
  elif not args.debug:
    f_pts1      = open(args.test_pts1_file, 'a')
    f_gt        = open(args.test_gt_file,   'a')
  ...
  for i in range(args.img_per_real):
    ...
    y = random.randint(rho, height - rho - patch_size)  # row? 45, 240-45-128 (45, 67)
    x = random.randint(rho, width - rho - patch_size)  # col?  45, 320-45-128 (45, 147)
...
def main():
  ...
  HEIGHT = 240                 # Size of synthetic image
  WIDTH = 320
  PATCH_SIZE = 128

  # Directories to files
  RAW_DATA_PATH = "C:/Users/xwen2/Desktop/HomoNet-Unsupervised homography estimation/data/train/" # Real images used for generating synthetic data
  TEST_RAW_DATA_PATH = "C:/Users/xwen2/Desktop/HomoNet-Unsupervised homography estimation/data/test/" # Real images used for generating test synthetic data
